...
class PyErg(object):
    ...
    def __init__(self, erg):
        ...
        try:
            ...
            # Errno 16 (Resource busy) is expected when device already configured - ignore it
            if e.errno != 16:
    ...
    def set_workout(self, program=None, workout_time=None, distance=None,
        ...
        powerpace: watts
        """
        # PM5 firmware 459+: Check status before sending workout
        print("DEBUG: Checking PM5 status before workout setup...")
        import sys
        import time
        sys.stdout.flush()
        
        try:
            status = self.send(['CSAFE_GETSTATUS_CMD'])
            state = status.get('CSAFE_GETSTATUS_CMD', [0])[0]
            state_names = ['Error','Ready','Idle','Have ID','N/A','In Use','Pause','Finished','Manual','Offline']
            print(f"DEBUG: PM5 state: {state} ({state_names[state] if state < len(state_names) else 'Unknown'})")
            sys.stdout.flush()
            
            if state != 1:  # Not in Ready state
                print(f"WARNING: PM5 not in Ready state (state={state}). Workout setup may fail.")
                print("Try resetting the PM5 or selecting 'New Workout' on the display.")
                sys.stdout.flush()
        except Exception as e:
            print(f"DEBUG: Could not get PM5 status: {e}")
            sys.stdout.flush()
        ...
        # PM5 firmware 459+: Send commands separately instead of batching
        print(f"DEBUG: Sending distance/time command: {command}")
        sys.stdout.flush()
        if command:  # Only send if there's a goal set
            self.send(command)
            time.sleep(0.2)  # Give PM5 time to process
        
        print(f"DEBUG: Sending SETPROGRAM ({program})...")
        sys.stdout.flush()
        self.send(['CSAFE_SETPROGRAM_CMD', program, 0])
        time.sleep(0.2)
        
        print("DEBUG: Sending GOINUSE...")
        sys.stdout.flush()
        self.send(['CSAFE_GOINUSE_CMD'])
        print("DEBUG: Workout setup complete")
        sys.stdout.flush()
    ...
    def send(self, message):
        ...
        Per CSAFE spec: "application to not send additional commands until either
        a response has been received or the minimum frame spacing has elapsed."
        This means: wait for (response OR MIN_FRAME_GAP) before NEXT send, not before THIS send.
        """
        
        # Acquire lock to ensure only one thread accesses USB at a time
        with self.__send_lock:
            # Per spec: wait for MIN_FRAME_GAP only if we haven't received a response yet
            # Since we always wait for response before returning, we only need to check
            # if MIN_FRAME_GAP has elapsed since the LAST RESPONSE (stored in __lastsend)
            now = datetime.datetime.now()
            delta = now - self.__lastsend
            deltaraw = delta.total_seconds()
            ...
            length = 0
            try:
                length = self.erg.write(self.outEndpoint, csafe, timeout=30000)  # 30s for PM5 459+
            # Checks for USBError 16: Resource busy
            except USBError as e:
                if e.errno != 19:
                    raise ConnectionError("USB device disconected")
                # If errno 19, set length to expected response size
                length = 120
            ...
            while not response:
                try:
                    ...
                    transmission = self.erg.read(self.inEndpoint, length, timeout=30000)  # 30s for PM5 459+
            ...
            # Record time when RESPONSE was received (not when command was sent)
            # This implements the spec requirement: "response has been received"
            self.__lastsend = datetime.datetime.now()
