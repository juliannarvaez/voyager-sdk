...
"""
Auto-calibrate One Euro Filter parameters from recorded keypoint data.

This script analyzes recorded keypoint JSON files and frame timing to estimate:
- sample rate (Hz)
- noise level (low-speed jitter)
- max speed (high-speed motion)

It then grid-searches min_cutoff and beta to satisfy a target precision and
minimize lag.

Usage:
  python calibrate_one_euro.py --input-dir /tmp/keypoints_YYYYMMDD_HHMMSS \
    --keypoint-idx 9 --detection-idx 0
"""
...
def _load_timestamps(timing_csv: Path) -> dict:
    timestamps = {}
    with timing_csv.open("r", newline="") as f:
...
def _extract_keypoint(json_path: Path, detection_idx: int, keypoint_idx: int, min_conf: float) -> Optional[Tuple[float, float]]:
    try:
    ...
    kpts = detections[detection_idx].get("keypoints")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Auto-calibrate One Euro Filter parameters from recorded keypoints")
    parser.add_argument("--input-dir", required=True, help="Directory containing keypoints_data and frame_timing.csv")
    parser.add_argument("--keypoint-idx", type=int, default=9, help="Keypoint index to use (default: 9)")
    parser.add_argument("--detection-idx", type=int, default=0, help="Detection index to use (default: 0)")
    parser.add_argument("--min-confidence", type=float, default=0.3, help="Minimum keypoint confidence (default: 0.3)")
    ...
    parser.add_argument("--max-lag-s", type=float, default=0.08, help="Maximum acceptable lag in seconds")
    ...
    timing_csv = input_dir / "frame_timing.csv"
    keypoints_dir = input_dir / "keypoints_data"

    if not keypoints_dir.exists():
        raise SystemExit(f"Missing keypoints_data directory in {input_dir}")

    timestamps_by_frame = _load_timestamps(timing_csv) if timing_csv.exists() else {}

    xs: List[float] = []
    ys: List[float] = []
    ts: List[float] = []

    json_files = sorted(keypoints_dir.glob("frame_*.json"))
    for jf in json_files:
        try:
            frame_num = int(jf.stem.split("_")[-1])
        except ValueError:
            continue
        point = _extract_keypoint(jf, args.detection_idx, args.keypoint_idx, args.min_confidence)
        if point is None:
            continue
        if timestamps_by_frame:
            t = timestamps_by_frame.get(frame_num)
            if t is None:
                continue
        else:
            t = float(frame_num)
    ...
    if not timestamps_by_frame:
        # If no timing file, assume 60 FPS for timestamps
        ts_arr = ts_arr / 60.0
