#!/usr/bin/env python3
"""SVD-factor the first ConvTranspose2d in ResNet-50 SimpleBaseline pose model."""
import os
import numpy as np
import onnx
from onnx import helper, TensorProto, numpy_helper

MODEL_IN = "customers/heatmap_pose/simplebaseline_res50_256x192.onnx"
MODEL_OUT = "customers/heatmap_pose/simplebaseline_res50_svd_deconv.onnx"
RANK = 512  # 512/4 = 128 MVM blocks, under 256 limit
...
# Only replace ConvTranspose nodes that exceed MVM limit
...
W2 = (sqrtS[:, None] * Vt_r)  # [RANK, C_out*k*k]

error = np.linalg.norm(W - W1 @ W2) / np.linalg.norm(W)
...
print(f"  Conv1x1({C_in}→{RANK}): {list(conv1x1_w.shape)} (MVM blocks={RANK//4})")
print(f"  ConvTranspose({RANK}→{C_out}, k={kH}): {list(convt_w.shape)} (MVM blocks={RANK//4})")
...
convt_node = helper.make_node(
    ...
    kernel_shape=[kH, kW], strides=[2, 2], pads=[1, 1, 1, 1], dilations=[1, 1], group=1,
)
...
new_graph = helper.make_graph(nodes, graph.name, list(graph.input), list(graph.output), all_inits)
...
try:
    ...
    print(f"Original: range=[{out_orig.min():.4f}, {out_orig.max():.4f}]")
    print(f"SVD:      range=[{out_new.min():.4f}, {out_new.max():.4f}]")
    print(f"Diff:     mean={diff.mean():.6f}, max={diff.max():.6f}")
    for k in range(17):
        op = np.unravel_index(out_orig[0,k].argmax(), out_orig[0,k].shape)
        np_ = np.unravel_index(out_new[0,k].argmax(), out_new[0,k].shape)
        if op != np_:
            print(f"  KP{k}: peak {op}→{np_}")
    print("[OK] Done")
except ImportError:
